...
template<class OP, class T>
void scanExcl(const unsigned int block_size,
              const unsigned long d_size,
              T* d_in,
              T* d_out){
    unsigned int num_blocks;
...
void sgmScanExcl(const unsigned int block_size,
    ...
    //shift it
    sgmShiftRightByOne<T><<<num_blocks, block_size>>>(sgmScanInc_res, flags, d_out, OP::identity(), d_size);
    cudaThreadSynchronize();
    cudaFree(sgmScanInc_res);
}
#endif //SCAN_HOST
