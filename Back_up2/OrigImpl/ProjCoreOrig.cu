...
rollback( const unsigned g, PrivGlobs& globs) {
    ...
    REAL* alist = (REAL*) malloc(outer*numX*numY*sizeof(REAL));
    REAL* blist = (REAL*) malloc(outer*numX*numY*sizeof(REAL));
    REAL* clist = (REAL*) malloc(outer*numX*numY*sizeof(REAL));
    REAL* ylist = (REAL*) malloc(outer*numX*numY*sizeof(REAL));
    ...
    REAL *d_myVarX, *d_myVarY, *d_myDxx, *d_myDyy, *d_myResult, *d_u, *d_v, 
         *d_alist, *d_blist, *d_clist, *d_ylist, *d_yy;
    ...
    cudaMalloc((void**)&d_alist, outer*numX*numY*sizeof(REAL));
    cudaMalloc((void**)&d_blist, outer*numX*numY*sizeof(REAL));
    cudaMalloc((void**)&d_clist, outer*numX*numY*sizeof(REAL));
    cudaMalloc((void**)&d_yy, outer*numZ*sizeof(REAL));
    cudaMalloc((void**)&d_ylist, outer*numX*numY*sizeof(REAL));
    ...
    cudaMemcpy(d_yy, yy, outer*numZ*sizeof(REAL), cudaMemcpyHostToDevice);
    ...
    implicitX<<<num_blocks, threadsPerBlock>>>(dtInv, d_myVarX, d_myDxx, d_alist,
                         d_blist, d_clist, numX, numY, expand);


    cudaThreadSynchronize();
    cudaMemcpy(alist, d_alist, outer*numX*numY*sizeof(REAL), cudaMemcpyDeviceToHost);
    cudaMemcpy(blist, d_blist, outer*numX*numY*sizeof(REAL), cudaMemcpyDeviceToHost);
    cudaMemcpy(clist, d_clist, outer*numX*numY*sizeof(REAL), cudaMemcpyDeviceToHost);
    
/*
    firstTridagLoop<<< num_blocks, threadsPerBlock>>>(d_alist, d_blist, d_clist, d_u, 
                                                d_yy, numZ, numZ_);
    cudaThreadSynchronize();

    cudaMemcpy(yy, d_yy, outer*numZ*sizeof(REAL), cudaMemcpyDeviceToHost);
    cudaMemcpy(u, d_u, outer*numX*numY*sizeof(REAL), cudaMemcpyDeviceToHost);
*/

    for(int h = 0; h<outer; h++) {
        for(j=0;j<numZ;j++) {
            tridag(&alist[h * expand + j*numZ_],&blist[h * expand + j*numZ_],
                   &clist[h * expand + j*numZ_],&u[h * expand + j*numZ_],
                    numZ_,&u[h * expand + j*numZ_],&yy[h*numZ]);
        }
    }

    cudaMemcpy(d_u, u, outer*numX*numY*sizeof(REAL), cudaMemcpyHostToDevice);
    cudaMemcpy(d_v, v, outer*numX*numY*sizeof(REAL), cudaMemcpyHostToDevice);
    ...
    for(int h=0; h<outer; h++) {
        for(i=0;i<numZ_;i++) {
            for(j=0;j<numZ;j++) {
                ylist[h * expand + i * numZ + j] = dtInv*u[h * expand + j * numZ_ + i] 
                                             - 0.5*v[h * expand + i * numZ + j];
            }
        }

     
        for(i=0;i<numZ_;i++) {
            tridag(&alist[h * expand + i*numZ],&blist[h * expand + i*numZ],
                   &clist[h * expand + i*numZ],&ylist[h * expand + i*numZ],
                    numY,&globs.myResult[h * expand + i*numZ],&yy[h*numZ]);
        }
    }
